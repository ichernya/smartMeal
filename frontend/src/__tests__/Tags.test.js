...
const server = setupServer(
  ...
  rest.put(URL + '/diets', (req, res, ctx) => {
    return res(
      ctx.status(200),
    );
  }),
...
test('update diet', async () => {
  ...
  fireEvent.click(screen.getByTestId('filterOpen'));

  waitFor(() => {
    const home = screen.getByText('Vegan');
    expect(home).toBeInTheDocument();
  });

  act(() => {
    fireEvent.click(screen.getByTestId('veganFalse'));
    fireEvent.click(screen.getByTestId('veganTrue'));
    fireEvent.click(screen.getByTestId('allFalse'));
    fireEvent.click(screen.getByTestId('allTrue'));
    fireEvent.keyDown(screen.getByTestId('tagsDrawer'), {code: 'Tab'});
    fireEvent.click(screen.getByTestId('closeTags'));
  });
});

...
test('email error', async () => {
  ...
  fireEvent.change(screen.getByTestId('email'),
    {target: {value: 'testdads@ucsc.edu'}});
  fireEvent.change(screen.getByTestId('password'),
    {target: {value: 'passworD1!'}});
  fireEvent.change(screen.getByTestId('confirmPassword'),
    {target: {value: 'passworD1!'}});
  act(() => {
    fireEvent.click(screen.getByTestId('Fab_SignUp'));
  });
  waitFor(() => {
    const signup = screen.findByText('Email already in use');
...
test('Sign up', async () => {
  ...
  fireEvent.click(screen.getByTestId('Fab_SignUp'));
  waitFor(() => {
    const signin = screen.findByText('Sign In');
